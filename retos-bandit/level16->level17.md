...
Se almacena la llave en el archivo "*llav*" dentro del directorio **/tmp/**;

Se cambian los permisos de la llave;

```bash
bandit16@bandit:/tmp$ chmod 400 llave.key
```

Ingresar con la llave primaria y revisar en la carpeta del passwoord para conocerlo

```bash
bandit16@bandit:/tmp$ ssh -i llave.txt bandit17@bandit.labs.overthewire.org -p2220
```
